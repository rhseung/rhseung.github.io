...
import { type TechStack, TechStackBadge } from './components/tech-stack-badge';

interface Project {
  title: string;
  description: string;
  techStacks: TechStack[];
  techStackKeys: (keyof typeof techStacks)[];
...
export const ProjectPage: React.FC = () => {
  ...
  const [selectedTechStacks, setSelectedTechStacks] = useState<
    (keyof typeof techStacks)[]
  >([]);

  const handleTechStackClick = (techKey: keyof typeof techStacks) => {
    setSelectedTechStacks((prev) =>
      prev.includes(techKey)
        ? prev.filter((key) => key !== techKey)
        : [...prev, techKey],
    );
  };
  ...
  const projects: Project[] = [
    ...
    },
    {
      ...
      techStackKeys: ['NodeJs', 'Npm', 'TypeScript'],
      links: {
    ...
    },
    {
      ...
      techStackKeys: ['Python', 'PyTorch', 'OpenCV'],
      links: {
    ...
    },
    {
      ...
      techStackKeys: ['Dart', 'Flutter', 'Nestjs', 'Prisma', 'Figma'],
      links: {
  ...
  const filteredProjects =
    selectedTechStacks.length === 0
      ...
      : projects.filter((project) =>
          selectedTechStacks.some((selectedTech) =>
            project.techStackKeys.includes(selectedTech),
          ),
        );
  ...
  return (
    <Layout>
      ...
      <div className="flex flex-col gap-10 py-16">
        ...
        <div className="max-w-7xl mx-auto px-16 lg:px-16 max-lg:px-6 flex gap-2 flex-wrap justify-center items-center">
          {Object.entries(techStacks).map(([key, tech]) => (
            <TechStackBadge
              key={key}
              tech={tech}
              size="md"
              className={cn(
                'cursor-pointer',
                selectedTechStacks.includes(key as keyof typeof techStacks) && [
                  'ring-2',
                  // 'ring-offset-2',
              ...
              onClick={() =>
                handleTechStackClick(key as keyof typeof techStacks)
              }
            />
          ))}
        </div>
